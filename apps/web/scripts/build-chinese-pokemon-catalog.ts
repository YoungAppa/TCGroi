 ...
 * triple-rare rate on the box back). That makes an *official-odds* ROI possible
 * later — but that lives in the pull-rate/products data, not here. This script
 * is catalog-only: searchable, holdable card rows with art.
 *
 * Like the JP build this is deliberately price-less for now (no free ZH price
 * feed); rows show "—" and sort last. Cards carry their Chinese names. Separate
 * keyspace from EN/JP (sets unique index is gameId+code+language), so no
 * collisions. Idempotent.
 *
 * Requires migration 0004 (set_language += 'ZH') applied first.
 *
...
const BASE = "https://api.tcgdex.net/v2/zh-cn";
...
async function main() {
  ...
  console.log(`${allSets.length} Simplified Chinese Pokémon sets on TCGdex; ingesting ${todo.length}.`);
  ...
  console.log(`\nDone: ${setsDone} ZH sets, ${cardsStored} cards stored (no prices — catalog only).`);
}
